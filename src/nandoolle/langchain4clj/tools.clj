...
(try
  (require '[nandoolle.langchain4clj.tools.schema :as schema-impl])
  (catch Exception _
    ;; Prismatic Schema not available
    nil))

(try
  (require '[nandoolle.langchain4clj.tools.malli :as malli-impl])
  (catch Exception _
    ;; Malli not available
    nil))
...
(defn detect-schema-type
  ...
  (cond
    ...
    (and (find-ns 'nandoolle.langchain4clj.tools.malli)
         ((resolve 'nandoolle.langchain4clj.tools.malli/malli?) schema)) :malli

    ;; Then Plumatic Schema
    (and (find-ns 'nandoolle.langchain4clj.tools.schema)
         ((resolve 'nandoolle.langchain4clj.tools.schema/schema?) schema)) :schema
  ...
  ([schema schema-type]
   (let [type (or schema-type (detect-schema-type schema))]
     (case type
       ...
       :schema (if (find-ns 'nandoolle.langchain4clj.tools.schema)
                 ((resolve 'nandoolle.langchain4clj.tools.schema/create-schema-provider) schema)
                 (throw (ex-info "Prismatic Schema not available. Add [prismatic/schema \"1.4.1\"] to deps.edn"
                                 {:schema-type :schema})))
       :malli (if (find-ns 'nandoolle.langchain4clj.tools.malli)
                ((resolve 'nandoolle.langchain4clj.tools.malli/create-malli-provider) schema)
